...
router.route('/')
	...
	.post((req, res) => { // create new category
		...
		resolve(`INSERT INTO menu_item_category VALUES (NULL, "${name}", "${detail}");`, res)
	})

	.put((req, res) => { // edit a category
		let { id, name, detail } = req.body

		resolve(`UPDATE menu_item_category SET name="${name}", detail="${detail}" WHERE id=${id}`, res)
	})

	.delete((req, res) => resolve(`DELETE FROM menu_item_category WHERE id=${req.body.id}`, res))
